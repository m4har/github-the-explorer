...
export const UserRepo = ({ user }: Props) => {
  ...
  const repoList = data || [];
  return (
    <AccordionItem value={user.id.toString()} data-testid={`user-repo-item-${user.id}`}>
      <AccordionTrigger className="bg-gray-200 px-2 my-2" data-testid={`user-repo-trigger-${user.id}`}>
        {user.login}
      </AccordionTrigger>
      <AccordionContent className="flex flex-col gap-4 text-balance px-2 ml-4">
        {isFetching ? (
          ...
          repoList.map((repo) => (
            <div className="flex flex-col gap-2 bg-gray-300 p-2" data-testid={`user-repo-${repo.id}`}>
